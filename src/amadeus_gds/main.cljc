(ns amadeus-gds.main
  "Kotodama WASM entrypoint for the Amadeus_gds clean-room actor (L4) — Clojure port.
  ...
  facts (`amadeus_gds.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "amadeus_gds")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"amadeus_gds.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "amadeus_gds-compat" :tier tier :entities entities} 200])
...
(defn start! [] :amadeus_gds-compat/ready)
